...
struct ContentView: View {
    ...
    var topBar: some View {
        VStack(spacing: 0) {
                    ...
                    HStack(spacing: 4) {
                        if proxy.isDirectMode {
                            Image(systemName: "antenna.radiowaves.left.and.right")
                                .font(.system(size: 9)).foregroundColor(Color.oAccent)
                        ...
                        if proxy.isDirectMode {
                            Text("Direct")
                                .font(.system(size: 10, weight: .medium)).foregroundColor(Color.oAccent)
                        } else if proxy.isCycling {
                            Text("Trying…")
            ...
            // Show current proxy / direct mode info
            if proxy.isReady {
                HStack(spacing: 4) {
                    if proxy.isDirectMode {
                        Image(systemName: "antenna.radiowaves.left.and.right").font(.system(size: 8)).foregroundColor(Color.oAccent)
                        Text("📡 Direct")
                        ...
                        Text("via \(proxy.best?.label ?? "unknown")")
                            .font(.system(size: 9, design: .monospaced)).foregroundColor(Color.oMuted)
            ...
            VStack(spacing: 14) {
                ...
                HStack {
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Text("Browse without a proxy").font(.system(size: 11)).foregroundColor(Color.oMuted)
                    }
                    Spacer()
                    Toggle("", isOn: Binding(
                        get: { proxy.isDirectMode },
                        set: { proxy.setDirectMode($0) }
                    ))
                    .labelsHidden()
                }
                ...
                HStack(spacing: 6) {
                    ...
                    Text(proxy.isDirectMode ? "📡 Direct Mode" : (proxy.isReady ? "Connected" : "Searching…"))
                        .font(.system(size: 13, weight: .semibold)).foregroundColor(Color.oText)
                }
                if proxy.isReady && !proxy.isDirectMode {
                    VStack(spacing: 4) {
                        Text(proxy.best?.label ?? "unknown")
                            .font(.system(size: 12, design: .monospaced)).foregroundColor(Color.oAccent)
                        if !proxy.workingProxies.isEmpty {
                            Text("\(proxy.workingProxies.count) working")
                ...
                if !proxy.isDirectMode {
                    ...
                    Button {
                        ...
                        Label("Find New Proxy", systemImage: "arrow.clockwise")
